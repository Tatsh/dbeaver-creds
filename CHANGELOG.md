...
- Prebuilt native release artefacts attached to each GitHub release:
  - macOS / Windows: ZIP archive per OS/architecture
    (`dbeaver-creds-<version>-{macos-x86_64,macos-arm64,windows-amd64,windows-arm64}.zip`)
    generated by CPack, containing the `bin/dbeaver-creds[.exe]` binary, `lib64/` static
    library, `include/` public header, `man/man{1,3}/` manpages, and
    `share/dbeaver-creds/{LICENSE.txt,README.md}` in a relocatable FHS layout.
  - Windows: NSIS installer
    (`dbeaver-creds-<version>-windows-{amd64,arm64}.exe`) installing to
  ...
  - Linux: Flatpak bundle
    (`sh.tat.dbeaver-creds-<version>-{x86_64,aarch64}.flatpak`), AppImage
    (`dbeaver-creds-<version>-{x86_64,aarch64}.AppImage`), and Snap
    (`dbeaver-creds_<version>_{amd64,arm64}.snap`). Each packages the native
    `dbeaver-creds` binary so end users have an installer that matches their distro's
    convention without building from source.
