...
Fixes mypy type errors and eliminates `Any` usage in the dbeaver-creds project.
...
1. Fix all mypy errors reported by `uv run mypy dbeaver_creds`
...
Place new TypedDict classes in `dbeaver_creds/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`dbeaver_creds/typing.py` when shared.
...
1. Run `uv run mypy dbeaver_creds` and capture all errors.
...
4. Run `uv run mypy dbeaver_creds` again to verify fixes.
