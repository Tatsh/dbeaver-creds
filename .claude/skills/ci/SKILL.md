...
- **changelog** - update `CHANGELOG.md` with entries for the changes.
  ...
  files under `dbeaver_creds/`, or dependency/version changes. **Skip for**: workflows, CI config, `.claude/`,
...
For example, if a commit contains `dbeaver_creds/main`, and `CHANGELOG.md`, the component is
