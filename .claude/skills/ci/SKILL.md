...
If any changed files are under `dbeaver_creds/` or `tests/`, run the following
agents **in order**:
...
1. **click-auditor** - validate Click command consistency. **Only run if files under
   `dbeaver_creds/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `dbeaver_creds/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `dbeaver_creds/commands/main.py`,
...
For Python files, strip the `dbeaver_creds/` prefix and replace `/` with `.` (like module imports).

- Python file `dbeaver_creds/media.py` → `media:`.
- Multiple files under `dbeaver_creds/commands/` → `commands:`.
- Single command file `dbeaver_creds/commands/admin.py` → `commands.admin:`.
